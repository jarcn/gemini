...
func (gr *GeminiResult) Create(db *sqlx.DB) (int64, error) {
	...
	insertQuery := `
        INSERT INTO tbl_gemini_result (cv_data, profile_data, cv_url, gemini_step1_result, gemini_step2_result, gemini_step3_result, gemini_step4_result, create_time, update_time, gemini_key)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `
...
func (gr *GeminiResult) Read(db *sqlx.DB, id int64) error {
	readQuery := `
        SELECT * FROM tbl_gemini_result WHERE id=?
    `
...
func (gr *GeminiResult) Delete(db *sqlx.DB) error {
	deleteQuery := `
        DELETE FROM tbl_gemini_result WHERE id=?
    `
