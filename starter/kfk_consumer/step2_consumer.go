...
func Step2ConsumerStart(brokers []string, topic, group string) {
	...
	handler := &Step2ConsumerGroupHandler{}                 // 处理函数
	...
	log.Println("Step2Consumer Starting ...")
...
// Step2ConsumerGroupHandler 实现了sarama.ConsumerGroupHandler接口
type Step2ConsumerGroupHandler struct{}

// Setup 在消费者组开始消费前调用，用于初始化
func (Step2ConsumerGroupHandler) Setup(sarama.ConsumerGroupSession) error { return nil }

// Cleanup 在消费者组停止消费后调用，用于清理
func (Step2ConsumerGroupHandler) Cleanup(sarama.ConsumerGroupSession) error { return nil }

// ConsumeClaim 将消息传递给处理程序处理
func (h *Step2ConsumerGroupHandler) ConsumeClaim(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
